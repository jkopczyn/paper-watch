...
| `backfill_relevance_scale.py` | Remap stored relevance from the old 0–4 rubric onto 0–10 (0/1/2/3/4 → 0/3/5/8/10); gate behavior on existing rows unchanged. **`--before ISO` restricts to entries first seen before the cutoff** — required once new-scale rows exist, since there's no scale marker (enrich_version was never bumped) and a blanket run would corrupt genuine new-scale 1–4s | applied 2026-08-05 (`--apply --before 2026-07-18T00:00:00Z`; 211/410 pre-cutoff rows rescaled) |
