...
- `new_window` (default 24h) / `max_new` (default 10): the digest leads with up to `max_new` never-shown papers first mentioned within `new_window`; remaining slots up to `top_n` are padded with resurfacing papers that outscore the new picks' average
- `recent_window` (default 48h): window for the "surfaced N×" chip on each digest item
- `candidate_window_days`: bounds the candidate pool — how recently a paper must have been mentioned to be considered at all (entering as *new* additionally requires a first mention within `new_window`); also the window over which recent mentions drive the velocity / 'buzz' signal and the surge test
...
- `schedule`: vestigial — nothing reads it; actual run times are set in `deploy/systemd/paper-watch.timer` (08:00/18:00)
